...
class RegisteredPersonaController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Iniciar sesión automáticamente con la PERSONA
            Auth::login($persona);

            // Redirigir con sesión flash
            return redirect()->route('register.persona')
                ->with('success', true)
                ->with('nombre', $persona->Nombre)
                ->with('apellido', $persona->Apellido)
                ->with('correo', $correo->Correo);
